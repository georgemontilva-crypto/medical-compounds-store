        ...
        <div className="relative h-48 cursor-pointer overflow-hidden bg-gradient-to-b from-[#f2f2f5] to-[#e8e8ed]">
          ...
          <button
            ...
            className={`absolute top-3 right-3 w-8 h-8 rounded-full shadow-md flex items-center justify-center transition-all duration-200 ${
              isAdded ? "bg-violet-600 text-white opacity-100 scale-110"
  ...
  return (
    <div className="min-h-screen bg-[#f8f8fa]">
      ...
      <div className="container py-10">
        {/* Page header */}
        <div className="mb-8">
          <p className="text-xs font-semibold tracking-widest uppercase text-violet-500 mb-1">Full Catalog</p>
          <h1 className="text-3xl font-extrabold text-gray-950">Research Compounds</h1>
          <div className="w-10 h-0.5 bg-gradient-to-r from-violet-500 to-indigo-400 mt-2 mb-3 rounded-full" />
          ...
          <aside className="hidden lg:flex flex-col gap-3 w-56 shrink-0 sticky top-[80px] self-start">
            <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
              ...
              >
                ...
                <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${!selectedCategory && !selectedStaticCat ? "bg-white/20 text-white" : "bg-gray-100 text-gray-500"}`}>
            ...
            {/* Find CTA */}
            <button className="w-full flex items-center justify-center gap-2 border border-violet-200 text-violet-600 text-xs font-semibold py-2.5 rounded-xl hover:bg-violet-50 transition-colors">
            ...
            <div className="flex items-center gap-3 mb-5">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-10 py-2.5 bg-white border border-gray-200 rounded-xl text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-violet-300 focus:border-violet-400 transition-all"
                />
                {search && (
                    ...
                    <div className="absolute right-0 top-full mt-1.5 w-52 bg-white border border-gray-100 rounded-xl shadow-lg z-20 overflow-hidden">
                      {Object.entries(SORT_LABELS).map(([val, label]) => (
                        <button key={val} onClick={() => { setSortBy(val as SortOption); setSortOpen(false); }}
                          className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${sortBy === val ? "bg-violet-50 text-violet-700 font-semibold" : "text-gray-700 hover:bg-gray-50"}`}>
                          {label}
                        </button>
                      ))}
                    </div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 mb-4 lg:hidden">
              <button
                ...
                className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors ${
                  !selectedCategory && !selectedStaticCat ? "bg-violet-600 text-white border-violet-600" : "bg-white text-gray-600 border-gray-200"
                }`}
              >
            ...
            {isLoading ? (
              ...
              filteredStatic.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-24 text-center">
                  ...
                  <button onClick={() => { setSearch(""); setSelectedStaticCat(null); }} className="mt-4 text-sm text-violet-600 font-semibold hover:underline">Clear filters</button>
                </div>
              ) : (
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
